...
def prepare_data(transcript_path, speakers, unit='token'):
    """
    ...
    dl = DataLoader(transcript_path, speakers)
    df = dl.load_transcripts(merge_speakers=True)
...
def do_preprocessing_sentences(df, pout=None):
    ...
    df['TEXT_PP_SENT'] = df.TEXT.apply(preprocess_text)
    return df
...
def batch_embed_tokens(model_path, df, output_dir_path, speakers, max_len=None):
    """

    :param model_path:
    :param df:
    :param output_dir_path:
    :param speakers: the speakers to include (P or both)
    :param max_len:
    :return:
    """
...
def batch_embed_sentences_with_padding(model_path, df, max_num_sent, max_sent_len, output_dir_path, speakers):
    ...
    for i, row in df.iterrows():
        ...
        pout = pout_dir + '/' + filename + '.pickle'
        pd.to_pickle(emb_sents, pout)


def batch_embed_sentences(model_path, df, output_dir_path,speakers):
    """
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m', '--model_path', type=str, nargs=1, help='path to word embedding model', required=True)
    ...
    df_data, max_len, max_num_sent = prepare_data(transcript_file, speakers, unit=unit)

    if unit in ['token', 'word']:
        batch_embed_tokens(model_path, df_data, pout, speakers)
    elif unit == 'sentence':
        batch_embed_sentences(model_path, df_data, pout, speakers)
    else:
